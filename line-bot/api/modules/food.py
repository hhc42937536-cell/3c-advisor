...
_CITY_DISTRICTS: dict[str, list[str]] = {
    ...
    "新竹": ["東區", "北區", "香山區", "竹北市", "竹東鎮", "新埔鎮", "關西鎮", "湖口鄉", "新豐鄉", "芎林鄉", "寶山鄉"],
    "苗栗": ["苗栗市", "頭份市", "竹南鎮", "後龍鎮", "通霄鎮", "苑裡鎮", "公館鄉", "三義鄉", "大湖鄉", "卓蘭鎮"],
    "彰化": ["彰化市", "員林市", "和美鎮", "鹿港鎮", "溪湖鎮", "田中鎮", "北斗鎮", "二林鎮", "社頭鄉", "永靖鄉"],
    "南投": ["南投市", "埔里鎮", "草屯鎮", "竹山鎮", "集集鎮", "名間鄉", "魚池鄉", "國姓鄉", "水里鄉", "信義鄉"],
    "雲林": ["斗六市", "斗南鎮", "虎尾鎮", "西螺鎮", "北港鎮", "土庫鎮", "二崙鄉", "崙背鄉", "麥寮鄉", "口湖鄉"],
    "嘉義": ["東區", "西區", "太保市", "朴子市", "布袋鎮", "大林鎮", "民雄鄉", "新港鄉", "六腳鄉", "竹崎鄉"],
    "屏東": ["屏東市", "潮州鎮", "東港鎮", "恆春鎮", "萬丹鄉", "內埔鄉", "里港鄉", "高樹鄉", "林邊鄉", "佳冬鄉"],
...
def build_destination_picker(city: str = "") -> list:
    """目的地美食：直接顯示城市層級結果，行政區用 quickReply 供細分。"""
    city2 = city[:2] if city else ""
    if not city2:
        return [{"type": "text",
                 "text": "請輸入目的地，例如：\n「目的地美食 台南」\n「目的地美食 板橋車站」"}]

    msgs = build_food_restaurant_flex(city2)
    districts = _CITY_DISTRICTS.get(city2, [])[:13]
    if msgs and districts:
        ...
        quick_items.append({"type": "action",
                             "action": {"type": "message",
                                        "label": "🏙️ 換縣市",
                                        "text": "目的地美食換城市"}})
...
def build_trending_specialty(city: str, mode: str) -> list:
    """必買伴手禮 / 最新流行：直接顯示城市層級結果，行政區用 quickReply 細分。"""
    city2 = city[:2] if city else ""
    if not city2:
        return []
    ...
    msgs = build_trending_by_district(city2, city2, mode)
