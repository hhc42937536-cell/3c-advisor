...
def build_food_area_picker(style: str) -> list:
    """今天吃什麼 — 選擇城市"""
    colors = {"健康": "#2E7D32", "享樂": "#C62828", "快速": "#E65100"}
    color = colors.get(style, "#E65100")
    icons = {"健康": "🥗", "享樂": "🍖", "快速": "⚡"}
    icon = icons.get(style, "🍽️")
    areas = ["台北", "新北", "桃園", "新竹", "台中", "台南", "高雄", "屏東", "宜蘭", "花蓮", "嘉義", "台東"]
    buttons = []
        ...
        buttons.append({
            ...
            "contents": [
                {"type": "button", "style": "secondary", "height": "sm", "flex": 1,
                 "action": {"type": "message", "label": a, "text": f"吃什麼 {style} {a}"}}
                for a in row
            ]
        })
    return [{"type": "flex", "altText": f"你在哪個城市？推薦{icon}{style}餐廳",
             "contents": {
                 "type": "bubble",
                 "header": {"type": "box", "layout": "vertical", "backgroundColor": color,
                            "contents": [
                                {"type": "text", "text": "🍽️ 你在哪個城市？",
                                 "color": "#FFFFFF", "size": "md", "weight": "bold"},
                                {"type": "text", "text": f"選擇後推薦你附近的{icon}{style}餐廳",
...
def build_food_message(text: str) -> list:
    ...
    # 有風格但沒有城市 → 先問城市
    has_style_kw = any(w in text_s for w in [
        "健康", "清爽", "低卡", "減脂", "快速", "方便", "省時", "懶得",
        "享樂", "好吃", "過癮", "大吃", "犒賞"
    ])
    is_internal = text_s.startswith("吃什麼 ")  # 內部按鈕觸發，格式為「吃什麼 風格 城市」
    if has_style_kw and not area and not is_internal:
        return build_food_area_picker(style)
                 ...
                 "footer": {"type": "box", "layout": "vertical", "spacing": "sm", "contents": [
                     ...
                     {"type": "box", "layout": "horizontal", "spacing": "sm", "contents": [
                         {"type": "button", "style": "link", "flex": 1, "height": "sm",
                          "action": {"type": "uri", "label": "📅 Accupass",
                                     "uri": f"https://www.accupass.com/search?q={urllib.parse.quote(area or '台灣')}&k=1"}},
                         {"type": "button", "style": "link", "flex": 1, "height": "sm",
                          "action": {"type": "uri", "label": "✈️ KKday",
                                     "uri": f"https://www.kkday.com/zh-tw/search?q={urllib.parse.quote((area or '')+'旅遊活動')}"}},
                         {"type": "button", "style": "link", "flex": 1, "height": "sm",
                          "action": {"type": "uri", "label": "🎯 Klook",
                                     "uri": f"https://www.klook.com/zh-TW/search/?query={urllib.parse.quote(area or '台灣')}"}},
                     ]},
                 ]},
             }}]
             ...
             "contents": {
                 ...
                 "footer": {"type": "box", "layout": "vertical", "spacing": "sm", "contents": [
                     ...
                     {"type": "box", "layout": "horizontal", "spacing": "sm", "contents": [
                         ...
                         {"type": "button", "style": "primary", "color": "#6A1B9A", "flex": 1,
                          "height": "sm", "action": {"type": "message", "label": "🎨 市集展覽", "text": "周末 市集展覽"}},
                     ]},
                     {"type": "button", "style": "primary", "color": "#AD1457", "height": "sm",
